...
define([
    ...
    return declare([itemDetailsHelper], {
        ...
        */
        startup: function () {
            var applicationHeaderDiv, tokenString;
            ...
            this._queryOrgBasemapGroup(tokenString);
        ...
        */
        _queryOrgBasemapGroup: function (tokenString) {
            var groupQuery = dojo.BaseMapGroupQuery;
            var groupUrl = dojo.configData.values.portalURL + "/sharing/rest/community/groups?q=" + groupQuery + "&f=json" + tokenString;
            this.searchPortalURL = dojo.configData.values.portalURL;
            this._fetchBaseMapGroupId(groupUrl);
        },

        /**
        * Fetch basemap group Id
        * @memberOf widgets/itemDetails/itemDetails
        */
        _fetchBaseMapGroupId: function (groupUrl) {
            var groupRequest;
            ...
            groupRequest.then(lang.hitch(this, function (groupInfo) {
                if (groupInfo.results.length === 0) {
                    ...
                    dojo.basemapGalleryGroupId = "";
                    return;
                }
                dojo.basemapGalleryGroupId = groupInfo.results[0].id;
            }), function (err) {
                alert(err.message);
                dojo.basemapGalleryGroupId = "";
                topic.publish("hideProgressIndicator");
        ...
        */
        addWebMap: function (mapId) {
            ...
            utils.createMap(mapId, this.itemMap, {
                ...
                home.startup();
                // If showBasemapGallery flag is set to true in the config file, show basemap gallery
                if (dojo.configData.values.showBasemapGallery) {
                    basemapGallery = new BasemapGallery({
                        map: this.map,
                        basemapGroupId: dojo.basemapGalleryGroupId
        ...
        */
        addLayerToMap: function (mapId, url, title, type, layers) {
            ...
            // if showBasemapGallery flag is set to true in config file create base map gallery
            if (dojo.configData.values.showBasemapGallery) {
                basemapGallery = new BasemapGallery({
                    map: this.map,
                    basemapGroupId: dojo.basemapGalleryGroupId
